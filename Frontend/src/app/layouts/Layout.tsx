  ...
  return (
    <SidebarProvider defaultOpen={true}>
      <div className="flex min-h-screen w-full bg-slate-50 dark:bg-slate-950">
        <Sidebar
          className="border-r border-slate-200 bg-white dark:bg-slate-900 dark:border-slate-800"
          variant="sidebar"
          collapsible="icon"
        >
          <SidebarHeader className="h-16 flex items-center px-4 border-b border-slate-100 dark:border-slate-800 group-data-[collapsible=icon]:px-0">
            <Link
              ...
              className="flex items-center gap-3 w-full group-data-[collapsible=icon]:justify-center hover:opacity-90 transition-opacity"
            >
        ...
        <SidebarInset className="flex-1 bg-slate-50 dark:bg-slate-950 min-h-screen flex flex-col transition-colors">
          <header className="h-16 flex items-center justify-between px-6 bg-white dark:bg-slate-900 border-b border-slate-200 dark:border-slate-800 sticky top-0 z-20 transition-colors">
            ...
            <div className="flex items-center gap-4">
              <GlobalSearch
                onNavigate={(path) => navigate(resolveAppPath(path))}
              />
              ...
              <Popover>
                <PopoverTrigger asChild>
                  <Button
                    variant="ghost"
                    size="icon"
                    className="relative text-slate-500 hover:text-blue-600 hover:bg-blue-50 dark:text-slate-400 dark:hover:bg-slate-800 dark:hover:text-blue-400"
                  >
                ...
                <PopoverContent
                  align="end"
                  className="w-80 p-0 rounded-xl shadow-xl dark:bg-slate-900 dark:border-slate-800"
                >
                  <div className="flex items-center justify-between p-4 border-b border-slate-100 dark:border-slate-800">
                    ...
                    <Tabs
                      defaultValue="all"
                      className="w-auto"
                      onValueChange={(v) => setNotifFilter(v as any)}
                    >
                      <TabsList className="h-7 bg-slate-100 dark:bg-slate-800 p-0.5">
                        <TabsTrigger
                          value="all"
                          className="text-[10px] h-6 px-2 rounded-sm"
                        >
                          {t("header.all")}
                        </TabsTrigger>
                        <TabsTrigger
                  ...
                  <div className="p-2 border-t border-slate-100 dark:border-slate-800">
                    ...
                    >
                      <Link to={appPaths.notifications}>
                        {t("header.view_all")}
                      </Link>
                    </Button>
                  </div>
      ...
      >
        <DialogContent className="dark:bg-slate-900 dark:border-slate-800">
          <DialogHeader>
            <DialogTitle className="flex items-center gap-2">
              {selectedNotification?.type === "RequestStatusUpdated" && (
                <span className="w-2 h-2 rounded-full bg-green-500" />
              )}
              {selectedNotification?.type === "RequestCreated" && (
