...
export const ClassDetailsView = ({
  ...
  const [editingSchedule, setEditingSchedule] = useState<TimeSlot | undefined>(
    undefined,
  );
    ...
    <div className="animate-in fade-in slide-in-from-right-4 duration-300">
      ...
      <div className="flex items-center gap-4 mb-6">
        <Button
          variant="ghost"
          onClick={onBack}
          className="pl-0 gap-2 text-slate-500 hover:text-slate-900 dark:text-slate-400 dark:hover:text-slate-100"
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {schedules
              ...
              .map((slot) => (
                <Card
                  key={slot.id}
                  className="hover:shadow-md transition-all border-slate-200 dark:border-slate-800 group"
                >
                  <CardHeader className="pb-3 flex flex-row items-start justify-between space-y-0">
                    <div>
                      <Badge
                        variant={
                          slot.type === "theoretical" ? "default" : "secondary"
                        }
                        className={
