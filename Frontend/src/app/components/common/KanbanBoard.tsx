...
export const DroppableColumn: React.FC<DroppableColumnProps> = ({
  ...
  const [{ isOver, canDrop }, dropRef] = useDrop<{ id: string; currentStatus: RequestStatus }, void, { isOver: boolean; canDrop: boolean }>(() => ({
    accept: ITEM_TYPE,
    drop: (item) => {
      ...
      <div className="space-y-4 flex-1">
        ...
        {requests.length === 0 && (
          <div className="text-center py-10 text-slate-400 text-xs italic border-2 border-dashed border-slate-200 dark:border-slate-800 rounded-lg">
            {isOver && canDrop ? `Drop to move to ${title}` : `No ${status} requests`}
          </div>
        )}
      </div>
    </div>
...
export const KanbanBoard: React.FC<KanbanBoardProps> = ({
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-3 gap-6 flex-1 h-full min-h-0 overflow-y-auto">
      <DroppableColumn
        status="pending"
