...
export const RequestCard: React.FC<RequestCardProps> = ({
  ...
  const [{ isDragging }, dragRef] = useDrag<{ id: string; currentStatus: RequestStatus }, unknown, { isDragging: boolean }>(() => ({
    type: ITEM_TYPE,
  ...
  const getComponentBadge = (type: string) => {
    switch (type) {
      case 'theoretical':
        return <span className="text-[10px] font-bold px-1.5 py-0.5 rounded bg-purple-100 text-purple-700 border border-purple-200">T</span>;
      case 'practical':
        return <span className="text-[10px] font-bold px-1.5 py-0.5 rounded bg-blue-100 text-blue-700 border border-blue-200">P</span>;
      default:
        return null;
    }
  };
          ...
          <div className="flex justify-between items-start gap-2">
            <div className="space-y-1">
              <div className="flex items-center gap-2 flex-wrap">
                ...
                {request.componentType && getComponentBadge(request.componentType)}
            ...
            <DropdownMenu>
              <DropdownMenuTrigger asChild>
                <Button variant="ghost" className="h-6 w-6 p-0 rounded-full hover:bg-slate-100 dark:hover:bg-slate-800 text-slate-400 dark:text-slate-500 hover:text-slate-600 dark:hover:text-slate-300 shrink-0">
              ...
              <DropdownMenuContent align="end" className="w-48 rounded-xl shadow-xl border-slate-100 dark:border-slate-800 dark:bg-slate-900">
                ...
                <DropdownMenuItem onClick={(e) => { e.stopPropagation(); onViewDetails(request); }} className="cursor-pointer">
                  View Details
                </DropdownMenuItem>

                {/* Teacher Actions */}
                {isTeacher && request.status === 'pending' && onEdit && (
                  <DropdownMenuItem onClick={(e) => { e.stopPropagation(); onEdit(request); }} className="cursor-pointer">
                    Edit Request
                  </DropdownMenuItem>
                )}
                {isTeacher && request.status === 'pending' && onCancel && (
                  <>
                ...
                {isCoordinator && request.status === 'pending' && onChangeStatus && (
                  <>
                    ...
                    <DropdownMenuItem
                      ...
                      onClick={(e) => { e.stopPropagation(); onChangeStatus(request.id, 'approved'); }}
                    >
                      Approve Request
                    </DropdownMenuItem>
                    <DropdownMenuItem
                      className="text-red-600 dark:text-red-400 focus:text-red-700 dark:focus:text-red-300 cursor-pointer"
                      onClick={(e) => { e.stopPropagation(); onChangeStatus(request.id, 'rejected'); }}
                    >
                      Reject Request
                    </DropdownMenuItem>
                  </>
