...
import { Search, LayoutGrid, Table as TableIcon, SlidersHorizontal, Filter, ArrowUpDown, Check } from 'lucide-react';
...
export type CourseFilter = 'all' | 'Computer Science' | 'Information Systems' | 'Design';
        ...
        <div className="flex items-center gap-2 shrink-0">
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button 
                variant="outline" 
                size="icon" 
                className={cn("h-9 w-9 rounded-xl border-slate-200 dark:border-slate-800 text-slate-600 dark:text-slate-400 dark:hover:bg-slate-800", courseFilter !== 'all' && "bg-purple-50 text-purple-600 border-purple-200 dark:bg-purple-900/20 dark:text-purple-400 dark:border-purple-800")}
              >
            ...
            <DropdownMenuContent align="end" className="w-48 dark:bg-slate-900 dark:border-slate-800">
              ...
              {(['all', 'Computer Science', 'Information Systems', 'Design'] as CourseFilter[]).map((cf) => (
                <DropdownMenuItem key={cf} className="cursor-pointer" onClick={() => onCourseFilterChange(cf)}>
                  <div className="flex items-center w-full">
                    <span className="flex-1">{cf === 'all' ? 'All Courses' : cf}</span>
                    {courseFilter === cf && <Check className="h-3.5 w-3.5 text-blue-600 dark:text-blue-400" />}
                  </div>
                </DropdownMenuItem>
              ))}
            </DropdownMenuContent>
          ...
          <DropdownMenu>
            <DropdownMenuTrigger asChild>
              <Button 
                variant="outline" 
                size="icon" 
                className={cn("h-9 w-9 rounded-xl border-slate-200 dark:border-slate-800 text-slate-600 dark:text-slate-400 dark:hover:bg-slate-800", statusFilter !== 'all' && "bg-blue-50 text-blue-600 border-blue-200 dark:bg-blue-900/20 dark:text-blue-400 dark:border-blue-800")}
              >
            ...
            <DropdownMenuContent align="end" className="w-48 dark:bg-slate-900 dark:border-slate-800">
              ...
              {(['all', 'pending', 'approved', 'rejected'] as StatusFilter[]).map((sf) => (
                <DropdownMenuItem key={sf} className="cursor-pointer capitalize" onClick={() => onStatusFilterChange(sf)}>
                  <div className="flex items-center w-full">
                    <span className="flex-1">{sf === 'all' ? 'All Requests' : sf}</span>
                    {statusFilter === sf && <Check className="h-3.5 w-3.5 text-blue-600 dark:text-blue-400" />}
                  </div>
                </DropdownMenuItem>
              ))}
            </DropdownMenuContent>
          ...
          <DropdownMenu>
            ...
            <DropdownMenuContent align="end" className="w-48 dark:bg-slate-900 dark:border-slate-800">
              ...
              <DropdownMenuSeparator className="bg-slate-100 dark:bg-slate-800" />
              <DropdownMenuItem className="cursor-pointer" onClick={() => onSortOrderChange('asc')}>
                <div className="flex items-center w-full">
                  <span className="flex-1">Oldest First</span>
                  {sortOrder === 'asc' && <Check className="h-3.5 w-3.5 text-blue-600 dark:text-blue-400" />}
                </div>
              </DropdownMenuItem>
              <DropdownMenuItem className="cursor-pointer" onClick={() => onSortOrderChange('desc')}>
                <div className="flex items-center w-full">
          ...
          >
            <TabsList className="h-9 bg-slate-100 dark:bg-slate-800 p-0.5 rounded-lg border border-slate-200 dark:border-slate-700">
              <TabsTrigger value="current" className="text-xs px-3 h-7 rounded-md">Current</TabsTrigger>
              <TabsTrigger value="history" className="text-xs px-3 h-7 rounded-md">History</TabsTrigger>
            </TabsList>
          </Tabs>
        </div>
      </div>
