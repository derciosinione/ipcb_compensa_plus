...
const toTimetableEvents = (
  ...
  const toEvent = (schedule: ClassSchedule): CalendarTimetableEvent => {
    const unit = details.units.find(
      (item) => item.id === schedule.curricularUnitId,
    );
    const classGroup = details.classes.find(
      (item) => item.id === schedule.classGroupId,
    );
...
export const CalendarPage = ({
    ...
    try {
      ...
      setTimetableEvents(
        details.flatMap((detail, index) =>
          detail
            ? toTimetableEvents(activeCourses[index], detail, loadedClassrooms)
            : [],
        ),
      );
    } catch (error) {
    ...
    if (calendarMode === "requests") {
      ...
      return timetableEvents
        .filter((slot) => {
          ...
          const matchesYear =
            filterYearGroup === "all" || slot.yearGroup === filterYearGroup;
          const matchesClass =
            filterClassGroup === "all" || slot.classGroup === filterClassGroup;
      ...
      const occupiedTimetableEvents = timetableEvents
        .filter((slot) => {
          const roomMatches =
      ...
      const requestEvents = requests
        .filter((req) => {
          const roomDetails = classrooms.find(
            (room) => room.name === req.newRoom || room.id === req.newRoom,
          );
          const roomMatches =
            filterRoom === "all" || roomDetails?.id === filterRoom;
          const typeMatches =
            filterRoomType === "all" ||
            (roomDetails && roomDetails.type === filterRoomType);
  ...
  const renderMonthCell = (date: Date | null, idx: number) => {
    if (!date)
      return (
        <div
          key={`empty-${idx}`}
          className="bg-slate-50/50 dark:bg-slate-900/50 border-b border-r border-slate-100 dark:border-slate-800/50 min-h-[120px]"
        />
      );
                ...
                event.status === "approved"
                  ...
                  : event.status === "rejected"
                    ...
                    : event.status === "pending"
                      ...
                      : // Occupancy styles
                        event.status === "occupied-class"
                        ? "bg-slate-100 text-slate-700 border-slate-200 dark:bg-slate-800 dark:text-slate-300 dark:border-slate-700"
                        : event.status === "occupied-request"
          ...
          {days.map((day) => {
            ...
            return (
              <div
                key={day.toISOString()}
                className={cn(
                  "flex-1 py-3 text-center border-r border-slate-100 dark:border-slate-800",
                  isDayHoliday && "bg-slate-50 dark:bg-slate-950/30",
                )}
              >
        ...
        <div className="flex-1 relative">
          ...
          <div className="absolute left-0 top-0 bottom-0 w-16 border-r border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-950/50 z-10">
            {hours.map((hour) => (
              <div
                key={hour}
                className="h-20 text-xs text-slate-400 text-right pr-2 pt-2 border-b border-slate-100 dark:border-slate-800 relative"
              >
                {hour}:00
          ...
          {/* Columns */}
          <div className="ml-16 flex h-[1040px]">
            {" "}
            {/* 13 hours * 80px */}
            {days.map((day) => {
              ...
              return (
                <div
                  key={day.toISOString()}
                  className={cn(
                    "flex-1 border-r border-slate-100 dark:border-slate-800 relative",
                    isDayHoliday && "bg-diagonal-stripes opacity-50",
                  )}
                >
                  {/* Grid Lines */}
                  {hours.map((hour) => (
                    ...
                    return (
                      <div
                        ...
                        className={cn(
                          ...
                          event.status === "approved"
                            ...
                            : event.status === "rejected"
                              ? "bg-red-100/90 border-red-200 text-red-800 dark:bg-red-900/50 dark:border-red-800 dark:text-red-200"
                              : event.status === "pending"
                                ? "bg-amber-100/90 border-amber-200 text-amber-800 dark:bg-amber-900/50 dark:border-amber-800 dark:text-amber-200"
                                : // Occupancy styles
                                  event.status === "occupied-class"
                                  ? "bg-slate-200/90 border-slate-300 text-slate-800 dark:bg-slate-700/80 dark:border-slate-600 dark:text-slate-200"
                                  : event.status === "occupied-request"
                                    ? "bg-purple-100/90 border-purple-200 text-purple-800 dark:bg-purple-900/50 dark:border-purple-800 dark:text-purple-200"
                      ...
                      >
                        ...
                        <div className="flex items-center gap-1 mt-1 opacity-80">
                          ...
                          {event.newTime.includes(" -")
                            ? event.newTime
                            : `${event.newTime} - ${event.endTime || `${h + 2}:00`}`}
                        </div>
  ...
  const getCalendarModeDescription = () => {
    switch (calendarMode) {
      case "requests":
        return t("calendar.manage_requests");
      case "timetable":
        return t("calendar.view_timetable");
      case "occupancy":
        return t("calendar.check_occupancy");
      default:
        return "";
    }
  };
  ...
  return (
    <div className="flex flex-col h-full animate-in fade-in duration-500">
            ...
            <SelectContent>
              <SelectItem value="requests">
                <span className="flex items-center gap-2">
                  ...
                  {t("calendar.mode_requests")}
                </span>
              </SelectItem>
              <SelectItem value="timetable">
                <span className="flex items-center gap-2">
                  <List className="w-4 h-4" /> {t("calendar.mode_timetables")}
                </span>
              </SelectItem>
              <SelectItem value="occupancy">
                <span className="flex items-center gap-2">
                  <Columns className="w-4 h-4" /> {t("calendar.mode_occupancy")}
                </span>
              </SelectItem>
            </SelectContent>
          </Select>
      ...
      <div className="flex items-center gap-3 mb-4 overflow-x-auto pb-2">
        <Badge
          variant="outline"
          className="text-slate-500 bg-slate-50 border-slate-200 h-8 px-3 flex items-center gap-2"
        >
          ...
          <>
            <Select value={filterYearGroup} onValueChange={setFilterYearGroup}>
              ...
              <SelectContent>
                ...
                {Array.from(
                  new Set(timetableEvents.map((event) => event.yearGroup)),
                ).map((yearGroup) => (
            ...
            >
              ...
              <SelectContent>
                ...
                {Array.from(
                  new Set(timetableEvents.map((event) => event.classGroup)),
                ).map((classGroup) => (
          ...
          <>
            <Select value={filterRoomType} onValueChange={setFilterRoomType}>
              ...
              <SelectContent>
                ...
                {Array.from(new Set(classrooms.map((room) => room.type))).map(
                  (type) => (
        ...
        {viewType === "month" ? (
          <div className="grid grid-cols-7 border-b border-slate-200 dark:border-slate-800">
            {["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map((day) => (
              <div
                key={day}
                className="py-2 text-center text-xs font-semibold text-slate-500 uppercase bg-slate-50 dark:bg-slate-950/50 border-r border-slate-100 dark:border-slate-800 last:border-r-0"
              >
                {day}
              </div>
            ))}
