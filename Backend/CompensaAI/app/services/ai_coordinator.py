...
class AI_Coordinator:
    ...
    async def send_message(self, thread_id: str, content: str, file_ids: Optional[List[str]] = None, user_context: Optional[Dict] = None) -> Dict[str, Any]:
        ...
        if not thread_id.startswith("gemini_thread_") and time.time() > self.openai_disabled_until:
            try:
                # OpenAI service is still sync for now, but we can call it in a thread if needed
                # For now, let's keep it simple as the user is mostly using Gemini
                ...
                if response.get("type") == "error":
                    ...
                    return await self._fallback_to_gemini(content, gemini_files, user_context)
                return response
            except Exception as e:
            ...
            return await gemini_service.send_message(thread_id, content, gemini_files, user_context)
